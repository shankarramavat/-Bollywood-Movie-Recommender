...
def load_data():
    ...
    if os.path.exists('bollywood_movies.pkl'):
        try:
            import pickle
            with open('bollywood_movies.pkl', 'rb') as f:
                df = pickle.load(f)
    ...
    if tmdb_api_key:
        try:
            ...
            df = fetch_bollywood_movies_from_tmdb(tmdb_api_key)
            if not df.empty:
                # Cache to disk for future use
                try:
                    import pickle
                    with open('bollywood_movies.pkl', 'wb') as f:
                        pickle.dump(df, f)
...
def fetch_bollywood_movies_from_tmdb(api_key):
    """
    ...
    # Search for Bollywood movies (using region=IN and with_original_language=hi for Hindi)
    movies_data = []
    
    # We'll get several pages of results
    for page in range(1, 6):  # Get 5 pages (100 movies total)
        # Search for popular Indian Hindi movies
        url = f"{base_url}/discover/movie?api_key={api_key}&with_original_language=hi&region=IN&sort_by=popularity.desc&page={page}"
    ...
    print(f"Fetched {len(df)} Bollywood movies from TMDB")
